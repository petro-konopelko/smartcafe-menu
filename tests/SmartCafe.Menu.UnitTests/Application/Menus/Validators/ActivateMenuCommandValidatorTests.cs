...
using SmartCafe.Menu.Application.Features.Menus.ActivateMenu.Validators;

namespace SmartCafe.Menu.UnitTests.Application.Menus.Validators;

public class ActivateMenuCommandValidatorTests
{
    [Fact]
    public void Validate_IsValid_WhenIdsProvided()
    {
        // Arrange
        var validator = new ActivateMenuCommandValidator();
        var command = new ActivateMenuCommand(Guid.NewGuid(), Guid.NewGuid());
    ...
    public void Validate_ReturnsErrors_WhenIdsMissing()
    {
        // Arrange
        var validator = new ActivateMenuCommandValidator();
        var command = new ActivateMenuCommand(Guid.Empty, Guid.Empty);
