...
using DomainMenu = SmartCafe.Menu.Domain.Entities.Menu;
...
public class MenuItemTests
{
    ...
    public void SyncMenu_ReturnsValidationError_WhenItemNameMissing(string? invalidName)
    {
        ...
        var menu = DomainMenu.Create(_cafeId, _faker.Company.CatchPhrase(), _idProvider, _clock, [section]).EnsureValue();
    ...
    public void Create_ReturnsValidationError_WhenItemNamesNotUniqueWithinSection(string itemName1, string itemName2)
    {
        ...
        // Act
        var result = DomainMenu.Create(_cafeId, _faker.Company.CatchPhrase(), _idProvider, _clock, sections);
    ...
    {
        // Arrange
        SectionUpdateInfo[] initialSections = [UpdateInfoDataGenerator.GenerateUpdateSectionInfo(itemCount: 2)];

        var menu = DomainMenu.Create(_cafeId, _faker.Company.CatchPhrase(), _idProvider, _clock, initialSections).EnsureValue();
    ...
    {
        // Arrange
        var initialSection = UpdateInfoDataGenerator.GenerateUpdateSectionInfo();
        var menu = DomainMenu.Create(_cafeId, _faker.Company.CatchPhrase(), _idProvider, _clock, [initialSection]).EnsureValue();
